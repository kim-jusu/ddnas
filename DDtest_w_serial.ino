...
void loop()
{
 ...
 /////추가한 부분 --> test 필!!/////////////////////////////////////////////////////////////////
    if(BTSerial.read()=='c'){//다른 아두이노가 level 1 감지시 본 아두이노에서도 알림음 울림
      alarmLev1();
    }
   
    if(BTSerial.read()=='d'){//다른 아두이노가 level 1 감지시 본 아두이노에서도 알림음 울림
      alarmLev2();
    }
    if(BTSeral.read()=='x'){
      noTone(Buzzer);//위험상황 종료의 경우 버저 끈다
    }
  }
//여기서부터 되는지 확인 안함 안드로이드로부터 알림신호 받았을경우 버저로 울리는거
